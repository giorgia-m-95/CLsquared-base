...
def sequence_cleaner(camp, meta, ambiguous, complete, tolerance, path_out):
    print("ENTERED ##########")
    ...
    with open(f"{path_out}CL_squared_step_02/{name}_cleaned_summary.txt", "w") as doc:
        with open(f"{path_out}CL_squared_step_02/Temp_FASTA/{name}_cleaned_step_02.fasta", "w") as cleaned:
...
parser = argparse.ArgumentParser(description="Sequence filtering on the basis of the user requests:\n- COMPLETENESS "
                                             ...
                                             "ATTENTION:If the user intends to generate multiple FASTA files from "
                                             "existing ones to run the pipeline in parallel, they can use the A.1 script"
                                             " of the pipeline.",
...
# User metadata file
parser.add_argument("usr_meta", type=str, help="User metadata file path. If you work with NCBI meta please download the"
                                               " accession ID WITH VERSION\nIf you have multiple metadata files, "
                                               "concatenate or merge them.\nThe metadata are used in order to quickly "
...
if __name__ == "__main__":
    ...
    if len(camps) == 1:
        sequence_cleaner(camps[0], meta, args.ambiguous, args.complete, args.tolerance, path_out)

    else:
        for camp in camps:
            sequence_cleaner(camp,
                             meta,
                             args.ambiguous,
                             args.complete,
                             args.tolerance,
